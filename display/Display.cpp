...
uint Display::IncrementDisplayCount()
{
    DisplayCount++;
    return DisplayCount;
}
...
uint Display::SetCursorPlacement(uint X, uint Y)
{
    CursorCol = (Y %= Screen.Width);
    CursorRow = (X %= Screen.Height);
    return 0;
}

uint Display::SetCursorPlacement(Position PosXY)
{
    CursorCol = (PosXY.PosY %= Screen.Width);
    CursorRow = (PosXY.PosX %= Screen.Height);
    return 0;
}

uint Display::GetCursorColumn()
{
    return CursorCol;
}

uint Display::SetCursorColumn(uint Y)
{
    CursorCol = (Y %= Screen.Width);
    return 0;
}

uint Display::GetCursorRow()
{
    return CursorRow;
}

uint Display::SetCursorRow(uint X)
{
    CursorCol = (X %= Screen.Height);
    return 0;
}

uint Display::IncrementCursorColumn(uint NrOfSpaces)
{
    CursorCol += NrOfSpaces;
    if (CursorCol >= Screen.Width % 8)
    {
        CursorCol = 0;
        CursorRow++;
        if (CursorRow >= Screen.Height % 8)
            CursorRow = 0;
    }
    return 0;
}

uint Display::DecrementCursorColumn(uint NrOfSpaces)
{
    CursorCol -= NrOfSpaces;
    if (CursorCol < 0)
        CursorCol = 0;
...
uint Display::IncrementCursorRow(uint NrOfSpaces)
{
    CursorCol += NrOfSpaces;
    if (CursorCol >= Screen.Width % 8)
    {
...
uint Display::DecrementCursorRow(uint NrOfSpaces)
{
    CursorRow -= NrOfSpaces;
    if (CursorRow < 0)
        CursorRow = 0;
    return 0;
}

uint Display::PixelPositionToCursorColumn(uint Y)
{
    return (Y %= Screen.Width %= 8);
}

uint Display::PixelPositionToCursorRow(uint X)
{
    return (X %= Screen.Height %= 8);
}

Position Display::PixelPositionToCursorPosition(Position PosXY)
{
    return Position{PosXY.PosX %= Screen.Height %= 8, PosXY.PosY %= Screen.Width %= 8};
}

uint Display::CursorColumnToPixelPosition()
{
    return CursorCol * 8;
}

uint Display::CursorRowToPixelPosition()
{
    return CursorRow * 8;
}

Position Display::CursorPositionToPixelPosition()
{
    return {CursorRow * 8, CursorCol * 8};
}
...
// Returns FrameArea
render_area& Display::GetRenderArea()
{
	return frame_area;
}
...
// For 128p wide, 32p high, the X is <0 ; 31> and the Y is <0 ; 127>
