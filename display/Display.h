...
public:
	...
	static uint IncrementDisplayCount();
	...
	uint SetCursorPlacement(uint X, uint Y);
	uint SetCursorPlacement(Position PosXY);

	uint GetCursorColumn();
	uint SetCursorColumn(uint Y);

	uint GetCursorRow();
	uint SetCursorRow(uint X);

	uint IncrementCursorColumn(uint NrOfSpaces = 1);
	uint DecrementCursorColumn(uint NrOfSpaces = 1);

	uint IncrementCursorRow(uint NrOfSpaces = 1);
	uint DecrementCursorRow(uint NrOfSpaces = 1);

	uint PixelPositionToCursorColumn(uint Y);
	uint PixelPositionToCursorRow(uint X);
	Position PixelPositionToCursorPosition(Position PosXY);

	uint CursorColumnToPixelPosition();
	uint CursorRowToPixelPosition();
	Position CursorPositionToPixelPosition();
